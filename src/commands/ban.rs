...
impl BanType {
    ...
    async fn ban(
        ...
        embed
            .set_author(
                author
                    .name(format!("{} {} Banned", user.tag(), self.get_name()))
                    .clone(),
            )
        ...
        match self {
            Self::Server => dm_embed.field(
                "Appeal Forum",
                format!("[Click to appeal]({})", CONFIG.appeal_forum),
                false,
            ),
        ...
        match self {
            Self::Server => {
                let is_banned = crate::consts::DATABASE
                    .fetch_unbans()
                    .iter()
                    .any(|x| x.id == user_id.0);
                if is_banned {
                    embed
                        .set_author(
                            author
                                .name(format!("{} {} Ban Updated", user.tag(), self.get_name()))
                                .clone(),
                        )
                        .color(0x0E87CC);
                    dm_embed
                        .title(format!("Your {} Ban was Updated", self.get_name()))
                        .color(0x0E87CC);
                    if let Some(unban_date) = unban_date {
                        crate::consts::DATABASE.modify_unban_date(*user_id.as_u64(), unban_date)?;
                    } else {
                        // Remove the scheduled unban to enforce the permanent duration
                        crate::consts::DATABASE
                            .remove_entry("ScheduledUnbans", *user_id.as_u64())?;
                    }
    ...
    fn get_unban_date<'a>(
        &self,
        duration_option: Option<String>,
    ) -> err_resp::Result<'a, Option<OffsetDateTime>> {
    ...
    pub async fn exec(
        ...
        let user = user_id.to_user(&ctx).await.map_err(|_| {
            ErrorResponse::with_title(
                "Invalid User",
                "Please validate your user option and try again.",
            )
        })?;
        ...
        if crate::consts::DATABASE
            .fetch_freezes_for(user_id.0)
            .is_some()
        {
            super::unfreeze::unfreeze_user(ctx, user_id)
                .await
                .map_err(|err| {
        ...
        let embed = self
            .ban(ctx, user, member, executor.user.id, unban_date, dmd, reason)
            .await
            .map_err(|err| {
