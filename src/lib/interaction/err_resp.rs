...
#[derive(Debug)]
#[non_exhaustive]
pub struct ErrorResponse<'a>(pub CreateInteractionResponseData<'a>, pub String);

pub type Result<'a, T> = std::result::Result<T, Box<ErrorResponse<'a>>>;

impl<'a> ErrorResponse<'a> {
    pub fn message<S: ToString>(message: S) -> Box<ErrorResponse<'a>> {
    ...
    pub fn with_title<S: ToString, T: ToString>(
        title: S,
        description: T,
    ) -> Box<ErrorResponse<'a>> {
        let mut resp = CreateInteractionResponseData::default();
    ...
    pub fn with_footer<S: ToString, T: ToString, U: ToString>(
        title: S,
        description: T,
        footer: U,
    ) -> Box<ErrorResponse<'a>> {
