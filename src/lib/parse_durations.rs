    ...
    fn from_str(text: &str) -> Result<Self, Self::Err> {
        ...
        for caps in PARSE_REGEX.captures_iter(text) {
            let num = caps
                ...
                .as_str()
                .parse::<u64>()
                .map_err(|_| DurationParseError)?;
            let multiplier = MULTIPLIERS
                .get(caps.get(2).unwrap().as_str())
...
mod test {
    ...
    fn parse() {
        assert_eq!(
            "30sec 1y".parse::<Duration>().unwrap().0,
            (30 + 60 * 60 * 24 * 365)
        );
        assert_eq!(
            "3sec 1hour 1day 2months".parse::<Duration>().unwrap().0,
            (3 + 60 * 60 + 60 * 60 * 24 + 60 * 60 * 24 * 30 * 2)
        );
        assert_eq!(
            "1s 1h 1d 1month".parse::<Duration>().unwrap().0,
            (1 + 60 * 60 + 60 * 60 * 24 + 60 * 60 * 24 * 30)
        );
        assert_eq!(
            "1 m1sec 1minn789ad -5h".parse::<Duration>().unwrap().0,
            (60 * 2)
        );
